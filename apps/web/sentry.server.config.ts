...
Sentry.init({
  // Falls back to the public DSN: only NEXT_PUBLIC_SENTRY_DSN is passed as a
  // build arg and set in CI, so without this the server and edge runtimes
  // initialise with no DSN and silently report nothing. A Sentry DSN is not a
  // secret -- it is already in the browser bundle -- so sharing it is fine.
